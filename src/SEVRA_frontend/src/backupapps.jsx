...
function SevraApp({ user, signInWithGoogle }) {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      if (userData) {
        ...
        userData = normalizeData(userData);
        
        setBalance({
          sevBalance: userData.sevBalance || 0,
          usdtBalance: userData.usdtBalance || 0,
        });
        setWalletAddress(isConnected && address ? address : "Not connected");
      } else {
  ...
  const fetchData = async () => {
    try {
      ...
      if (user && address) {
        try {
          let balance = await backend.getUserBalance(user.uid, address);
          // Normalize to ensure no BigInt values
          balance = normalizeData(balance);
          
          setBalance(balance || { sevBalance: 0, usdtBalance: 0 });
          
          // Extract artwork token balances
          if (balance && balance.artTokens) {
            const tokenBalances = {};
            balance.artTokens.forEach(([id, amount]) => {
              tokenBalances[id] = amount;
            });
            setArtTokenBalances(tokenBalances);
          }
        } catch (error) {
      ...
      try {
        // Fetch order book for each artwork
        const activeArtworkId = transactionDetails.artworkId || (artworks[0]?.id || "UTD");
        let orderBook = await backend.getOrderBook(activeArtworkId);
        // Normalize to ensure no BigInt values
        orderBook = normalizeData(orderBook);
        
        setOrderBook(orderBook || { buyOrders: [], sellOrders: [] });
      } catch (error) {
    ...
    <div style={{ backgroundColor: "black" }}>
      ...
      <nav className="navbar">
        ...
        <div className="nav-center">
          <button className="nav-button" onClick={() => switchContainer('container2')}>ASSETS</button>
      ...
      {activeContainer === 'container1' && (
        <div id="container1">
          <video autoPlay muted loop id="video-background" playbackRate="0.7">
            <source src="Videos/stock_Container1.mp4" type="video/mp4" />
          </video>
      ...
      {/* Add a management panel for USDT and SEV */}
      {user && isConnected && (
