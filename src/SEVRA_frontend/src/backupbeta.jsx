    ...
    try {
        ...
        const processedOrderBook = Array.isArray(orderBookData)
            ? orderBookData.reduce(
                (acc, order) => {
                    if (order.artworkId === activeArtworkId) { // Filter berdasarkan artworkId aktif
                        const normalizedOrder = {
                            id: order.id || '',
                            user: order.user || '',
                            artworkId: order.artworkId || '',
        ...
        // Tambahkan staged orders ke order book (hanya untuk artwork aktif)
        ...
        for (const artwork of result) {
            ...
            const validateNumber = (value) =>
                isNaN(Number(value)) || value < 0 ? 0 : Number(value);
            tokenomicsData[artwork.id].allocation = {
                whale: validateNumber(tokenomicsResult.whale || 0),
                medium: validateNumber(tokenomicsResult.medium || 0),
                retail: validateNumber(tokenomicsResult.retail || 0),
                burned: validateNumber(tokenomicsResult.burned || 0),
                developer: validateNumber(tokenomicsResult.developer || 0)
            };
            // Hitung market cap dalam USDT (1 SEV = 20 USDT)
            const marketCap = validateNumber(tokenomicsResult.marketCap || 0);
            animatedValuesData[artwork.id] = {
                currentPrice: Number(artwork.currentPrice || 0),
                previousPrice: animatedValues[artwork.id]?.currentPrice || Number(artwork.currentPrice || 0),
                marketCap: marketCap * 20, // Konversi ke USDT (1 SEV = 20 USDT)
                previousMarketCap: animatedValues[artwork.id]?.marketCap || (marketCap * 20),
  ...
  const handlePlaceOrder = async (artworkId, amount, price, isBuying = true) => {
    ...
    try {
        ...
        if (result?.ok) {
            ...
            await fetchData(); // Update data langsung
            await fetchStagedOrders(); // Update staged orders
            setRefreshTrigger(prev => prev + 1);
            // Tambahkan kelas untuk efek visual
            const marketContent = document.querySelector(`.market-content[data-artwork="${artworkId}"]`);
            if (marketContent) {
                marketContent.classList.remove('buy-active', 'sell-active');
                if (isBuying) {
                    marketContent.classList.add('buy-active');
                } else {
                    marketContent.classList.add('sell-active');
                }
                setTimeout(() => marketContent.classList.remove('buy-active', 'sell-active'), 2000); // Hapus efek setelah 2 detik
            }
            // Tambahkan notifikasi untuk order bertahap (whale/medium)
            ...
            if (stagedOrder && (stagedOrder.userType === '#Whale' || stagedOrder.userType === '#Medium')) {
                addNotification(`SEV_${artworkId}: Your ${isBuying ? 'buy' : 'sell'} order will split because you are a ${stagedOrder.userType === '#Whale' ? 'whale' : 'medium'}. Next phase of ${isBuying ? 'buy' : 'sell'}: ${new Date(stagedOrder.nextStageTime / 1000000).toLocaleString()}`);
            }
        } else {
                        ...
                        <div className="market-stats">
                          ...
                          <div className="stat-item">
                              ...
                              <span className={Number(artwork.currentPrice) > Number(artwork.lastPrice) ? 'positive' : 'negative'}>
                                  ...
                                  {((Number(artwork.currentPrice) - Number(artwork.lastPrice)) / Number(artwork.lastPrice) * 100).toFixed(1)}%
                              </span>
                          </div>
                          <div className="stat-item">
                          ...
                          <div className="stat-item">
                              ...
                              <span className={`animated-number ${animatedValues[artwork.id]?.marketCap > animatedValues[artwork.id]?.previousMarketCap ? 'positive' : 'negative'}`}>
                                  <CountUp
                                      ...
                                      duration={1.5}
                                      separator=","
                                      suffix=" USDT" // Ubah suffix ke USDT karena 1 SEV = 20 USDT
                                      decimals={1} // Pastikan 1 desimal
                                      useEasing={true}
                                  />
                              </span>
